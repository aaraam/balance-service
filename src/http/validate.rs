...
fn validate_work_units(req: &BalanceRequest) -> Result<(), ApiError> {
    // Count total EVM token contracts across groups (excluding sol)
    let evm_tokens_total: usize = req.contracts
        .iter()
        .filter(|c| c.network_name.as_str() != "sol")
    ...
    if evm_work > MAX_EVM_WORK_UNITS {
        return Err(
            ApiError::bad_request(
                "INVALID_REQUEST_LIMITS",
                format!("EVM request too large (wallets * tokens > {})", MAX_EVM_WORK_UNITS),
                Some(
                    json!({
    ...
    // Count sol mints requested
    let sol_mints_total: usize = req.contracts
        .iter()
        .find(|c| c.network_name.as_str() == "sol")
        .map(|c| c.contract_addresses.len())
        .unwrap_or(0);

    let sol_work = req.solana_wallet_addresses.len().saturating_mul(sol_mints_total);
    if sol_work > MAX_SOL_WORK_UNITS {
        return Err(
            ApiError::bad_request(
                "INVALID_REQUEST_LIMITS",
                format!("Solana request too large (wallets * mints > {})", MAX_SOL_WORK_UNITS),
                Some(
                    json!({
...
fn _find_group<'a>(contracts: &'a [ContractGroup], network: &str) -> Option<&'a ContractGroup> {
    contracts.iter().find(|c| c.network_name.as_str() == network)
}
