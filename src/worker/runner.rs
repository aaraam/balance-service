...
async fn process_job(state: &AppState, request_key: &str) -> Result<(), anyhow::Error> {
    ...
    for cg in &req.contracts {
        ...
        // IMPORTANT:
        // Solana is handled by the SOL worker pipeline.
        // The EVM worker must NOT write stub totals for "sol", otherwise merged output becomes messy.
        if net == "sol" {
            continue;
        }
